 ...
 * 
 * Modified and adapted to work properly with the rest of FECR libraries in a generic way.
...
class FirmadorCR {
    ...
    private function retC14DigestSha1($strcadena) {
    ...
    /**
     * Signs an xml using Xades-EPES using the certificate provided.
     * 
     * @param string $certificadop12 Path to the file
     * @param string $clavecertificado 4 digit key code
     * @param string $xmlsinfirma The XML string, not encoded
     * @param int $tipodoc
     * @return type
     */
    public function firmar($certificadop12, $clavecertificado, $xmlsinfirma, $tipodoc = IDP::FE) {
        if (!$pfx = file_get_contents($certificadop12)) {
            throw new \Exception("Error: No se puede leer el fichero del certificado o no existe en la ruta especificada");
        }
        $key = null;
        if (openssl_pkcs12_read($pfx, $key, $clavecertificado)) {
            ...
            throw new \Exception("Error: No se puede leer el almacén de certificados o la clave no es la correcta");
        }
     ...
     */
    private function insertaFirma($xml) {
        ...
        $signTime1 = date("c");  //ISO 8601 date
